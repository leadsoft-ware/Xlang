...
bool isNormalExpression(ASTree ast){
    if(ast.this_node.type != TOK_DOT && ast.this_node.type != Id && ast.this_node.type != TOK_ID && ast.this_node.type != TOK_STRING && ast.this_node.type != TOK_ADDRBLOCK) return true;
    return false;
}
...
ASMBlock dumpToAsm(ASTree ast,int mode = false/*default is cast mode(0),but in global,it's global mode(1)*/){
    ...
    if(ast.nodeT == Id){
        ...
        if(symbol_table.find(ast.this_node.str) != symbol_table.end()){
            if(ast.this_node.type != TOK_PTRID || (ast.this_node.type == TOK_DOT && ast.node[0].this_node.type != TOK_PTRID)){
                ASMBlock ret = ASMBlock().genCommand("mov").genArg("reg"+std::to_string(getLastUsingRegId())).genArg("regsb").genCommand("sub").genArg("reg"+std::to_string(getLastUsingRegId())).genArg("regfp").genCommand("sub").genArg("reg" + std::to_string(getLastUsingRegId())).genArg(std::to_string(symbol_table[ast.this_node.str].frame_position + getMemberSize(ast))).push();
                if(mode == ArgumentsProcessMode) ret.genCommand("add").genArg("reg" + std::to_string(getLastUsingRegId())).genNumArg(289).push();
                return ret;
            }
    ...
    if(ASTree_APIs::MemberExpression::hasFunctionCallStatement(ast)){
        ...
        if(ast.this_node.type == TOK_DOT){
            ...
            asb += dumpToAsm(fullname,ArgumentsProcessMode); // 结构体传址
        ...
        if(args.nodeT == Args){
            ...
            for(int i = 0;i < args.node.size();i++){ 
                asb += dumpToAsm(args.node[i],ArgumentsProcessMode);
    ...
    if(ast.nodeT == ExpressionStatement){
        if(ast.this_node.type == TOK_PLUS || ast.this_node.type == TOK_MINUS || ast.this_node.type == TOK_MULT || ast.this_node.type == TOK_DIV || ast.this_node.type == TOK_2EQUAL || ast.this_node.type == TOK_NOTEQUAL || ast.this_node.type == TOK_MAXEQUAL || ast.this_node.type == TOK_MINEQUAL || ast.this_node.type == TOK_MAX || ast.this_node.type == TOK_MIN){
            ...
            else if(getMemberSize(ast.node[0]) == 8 && IsTakeAddressStatement(ast.node[0])) {
                //if(symbol_table.find(ast.node[0].this_node.str) != symbol_table.end() && mode == ArgumentsProcessMode) ab.genCommand("add").genArg("reg" + std::to_string(getLastUsingRegId() - 2)).genNumArg(289).push();
                ab.genCommand("mov").genArg("reg" + std::to_string(getLastUsingRegId() - 2)).genArg("[reg" + std::to_string(getLastUsingRegId() - 2) + "]").push();
            }
            ...
            else if(getMemberSize(ast.node[1]) == 8 && IsTakeAddressStatement(ast.node[1])) {
                //if(symbol_table.find(ast.node[1].this_node.str) != symbol_table.end() && mode == ArgumentsProcessMode) ab.genCommand("add").genArg("reg" + std::to_string(getLastUsingRegId() - 1)).genNumArg(289).push();
                ab.genCommand("mov").genArg("reg" + std::to_string(getLastUsingRegId() - 1)).genArg("[reg" + std::to_string(getLastUsingRegId() - 1) + "]").push();
            }
            else throw CompileError("TypeError: 加减乘除以及逻辑运算仅限于基础类型,除非你想让虚拟机崩掉.\nBasic operator and boolean expression only support basic types,if you want to let the vm crash then don't do it.");
        ...
        if(ast.this_node.type == TOK_EQUAL){
            ...
            if(ast.node[1].this_node.type == TOK_INTEGER || ast.node[1].this_node.type == TOK_DOUBLE || ast.node[1].this_node.type == TOK_CHARTER || ast.node[1].this_node.type == TOK_STRING || (ast.node[1].nodeT == ExpressionStatement && ast.node[1].this_node.type != TOK_DOT) || ast.node[1].this_node.type == TOK_ADDRBLOCK) /*do nothing*/;
            else realarg0 = "[" + realarg0 + "]";
                ...
                if(ast.node[i].nodeT == ExpressionStatement && ast.node[i].this_node.type == TOK_EQUAL){
                    ...
                    if(ast.node[i].node[0].nodeT == ArrayStatement){
                        ...
                        if(mode){
                            ...
                            symbol_table[ast.node[i].node[0].this_node.str] = Symbol(arrayt);
                            //sp -= arrayt.size; 不需要这一行
                        }
                        ...
                        for(long f = 0;f < ast.node[i].node[1].node.size();f++){
                            ...
                            RegState[getLastUsingRegId()] = true;
                            if(mode) asb += dumpToAsm(thisast);
                            else asb += dumpToAsm(ast.node[i].node[1].node[ast.node[i].node[1].node.size() - 1 - f]);
                            ...
                            if(IsTakeAddressStatement(thisast) && (guessType(thisast) == "char" || typen.name == "char")){
                                asb.genCommand("push1b").genArg("reg"+std::to_string(getLastUsingRegId())).genArg("[reg"+ std::to_string(getLastUsingRegId()) + "]").push();
                            }else if(guessType(thisast) == "char"){
                                asb.genCommand("push1b").genArg("reg"+std::to_string(getLastUsingRegId())).genArg("reg"+ std::to_string(getLastUsingRegId())).push();
                            }
                            //if(IsTakeAddressStatement(thisast)) asb.genCommand("mov_m").genArg("reg"+std::to_string(getLastUsingRegId())).genArg("[reg"+std::to_string(getLastUsingRegId())+"]").genNumArg(typen.size).push();
                            if(mode && IsTakeAddressStatement(thisast)) asb.genCommand("mov_m").genArg("[reg" + std::to_string(getLastUsingRegId()-1) + "]").genArg("[reg"+std::to_string(getLastUsingRegId())+"]").genNumArg(typen.size).push();
                            else if(mode) asb.genCommand("mov").genArg("[reg" + std::to_string(getLastUsingRegId()-1) + "]").genRegArg(getLastUsingRegId()).push();
                            else if(IsTakeAddressStatement(thisast)) asb.genCommand("push").genArg("[reg" + std::to_string(getLastUsingRegId()) + "]").genNumArg(typen.size).push();
                            else asb.genCommand("push").genArg("reg" + std::to_string(getLastUsingRegId())).genNumArg(typen.size).push();
                            RegState[getLastUsingRegId()-1] = false;
                    ...
                    if(mode){
                        std::string realarg0 = "reg" + std::to_string(getLastUsingRegId());
                        //std::cout << "BUGHERE:" << AST_nodeType[ast.node[i].node[0].nodeT] <<  std::endl;
                        if(ast.node[i].node[1].this_node.type == TOK_INTEGER || ast.node[i].node[1].this_node.type == TOK_DOUBLE || ast.node[i].node[1].this_node.type == TOK_CHARTER || ast.node[i].node[1].this_node.type == TOK_STRING || (ast.node[i].node[1].nodeT == ExpressionStatement && ast.node[i].node[1].this_node.type != TOK_DOT) || ast.node[i].node[1].this_node.type == TOK_ADDRBLOCK) /*do nothing*/;
                        else realarg0 = "[" + realarg0 + "]";
